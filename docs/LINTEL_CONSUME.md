...
- `daa2d8b` consume choreoir 0.1.13: CCE Copy indexed by layout stride; NVIDIA land `%k` `sha256:20785880…` / Ascend sibling `sha256:35a48584…`; copy cubin digest still `07aadb61…`; copy NPU-bin `e96d0b5d…`; gemm NPU-bin `e311b13c…`

Land those commits on lintel `origin/main` when write exists. Do not add `src/` or vendor `choreoir`. A throwaway `git am` of that 22-commit series onto origin `fda2db2` applies clean: year-1 slots `copy` / `gemm_tile`, freeze `artifact.kind=cubin` (`choreoir==0.1.13;nvcc.cubin`) plus `artifact.launch`, session events, no `src/`.
